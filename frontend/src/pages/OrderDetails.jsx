...
const OrderDetails = () => {
  ...
  const getExpectedDeliveryDate = () => {
    if (orderDetails.isDelivered) {
      if (orderDetails.deliveredAt) {
        ...
        if (!isNaN(deliveredDate)) {
          return `Delivered at ${deliveredDate.toLocaleString()}`;
        }
      }
      return 'Delivered';
    }
    ...
    if (orderDetails.expectedDelivery) {
      const expectedDateObj = new Date(orderDetails.expectedDelivery);
      if (!isNaN(expectedDateObj)) {
        return `Expected Delivery: ${expectedDateObj.toLocaleDateString()}`;
      }
    }

    // fallback to createdAt + 10 days
